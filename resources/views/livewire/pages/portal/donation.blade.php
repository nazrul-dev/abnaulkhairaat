<div>
    <div class="grid md:grid-cols-4 gap-5 mb-5">
        <div class="border rounded-lg bg-indigo-900 p-5">
            <div class="flex item-center gap-5">
                <div class="w-12 bg-indigo-200 rounded-lg flex flex-col items-center justify-center">
                    <x-icon name="gift" solid class="w-8 h-8 text-indigo-900" />
                </div>
                <div class="leading-5">
                    <div class="text-gray-200 text-sm font-semibold">
                        Total Donasi <span class="uppercase">Semua</span>
                    </div>
        ...
        <div class="border rounded-lg bg-emerald-900 p-5">
            <div class="flex item-center gap-5">
                ...
                <div class="leading-5">
                    ...
                    <div class="leading-2 text-lg text-white font-bold">
                        {{ $stat['paid'] }} / @currency($stat['total_paid'])
                    </div>
                </div>
            </div>
        </div>
        <div class="border rounded-lg bg-rose-900 p-5">
            <div class="flex item-center gap-5">
                <div class="w-12 bg-rose-200 rounded-lg flex flex-col items-center justify-center">
                    <x-icon name="gift" solid class="w-8 h-8 text-rose-900" />
                </div>
                <div class="leading-5">
                    <div class="text-gray-200 text-sm font-semibold">
                        Total Donasi UNPAID
                    ...
                    <div class="leading-2 text-lg text-white font-bold">
                        {{ $stat['unpaid'] }} / @currency($stat['total_unpaid'])
                    </div>
                </div>
            </div>
        </div>
        <div class="border rounded-lg bg-slate-900 p-5">
            <div class="flex item-center gap-5">
                <div class="w-12 bg-slate-200 rounded-lg flex flex-col items-center justify-center">
                    <x-icon name="gift" solid class="w-8 h-8 text-slate-900" />
                </div>
                <div class="leading-5">
                    <div class="text-gray-200 text-sm font-semibold">
                        Total Donasi EXPIRED
                    </div>
                    <div class="leading-2 text-lg text-white font-bold">
                        {{ $stat['expired'] }} / @currency($stat['total_expired'])
                    </div>
                </div>
            </div>
        </div>
    ...
    <div >
       <div class="p-2 sticky top-0 z-10">
        <x-button class="md:w-auto w-full" wire:click='addDonation()' icon="plus" positive>
            Donasi
        </x-button>
       </div>

        <div class="gap-2  grid md:grid-cols-2   p-2">

            @if ($transaksi->total())
                @foreach ($transaksi as $item)
                    <div class="bg-white p-5 rounded-lg h-auto">
                        <div class="flex justify-between md:items-center border-b pb-2 mb-3">
                            <div class="font-semibold">
                                #{{ $item->reference }}
                                <div class="md:hidden block">
                                    @if ($item->status === 'UNPAID')
                                        Status Pembayaran : <x-badge negative label=" {{ $item->status }}" />
                                    @else
                                        Status Pembayaran : <x-badge emerald label=" {{ $item->status }}" />
                                    @endif
                                </div>
                            </div>
                            <div class="flex md:flex-row flex-col gap-2">
                                <x-button sm icon="eye" wire:click='handlerDetail({{ $item }})' positive>
                                    Detail
                                </x-button>
                                @if ($item->status === 'UNPAID')
                                    <x-button href="{{ route('payments.checkout', $item->merchant_ref) }}" sm
                                        icon="cash" positive>
                                        Bayar
                                    </x-button>
                                @endif
                        ...
                        <div class="flex md:justify-between items-center gap-10">
                            <div>
                                <div>
                                    <strong class="md:text-xl">
                                        @currency($item->jumlah_donasi)</strong> / <small>Via Transfer</small> <strong>
                                        {{ $item->payment_name }}</strong>
                                </div>
                                <div class="text-sm">
                                    @if ($item->status === 'UNPAID')
                                        <p class="text-rose-500  animate-pulse"> Lakukan Pembayaran sebelum
                                            {{ Carbon\Carbon::createFromTimestamp($item->expired_time)->format('m-d-Y h:i') }}
                                        </p>
                                    @else
                                        Tanggal Donasi : {{ $item->created_at }}
                                    @endif
                            ...
                            <div class="md:block hidden">
                                @if ($item->status === 'UNPAID')
                                    <x-badge negative label=" {{ $item->status }}" />
                                @else
                                    <x-badge emerald label=" {{ $item->status }}" />
                                @endif
            ...
            @else
                <div class="md:col-span-2">
                    <div class="flex flex-col items-center justify-center p-20 text-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-48 w-48" data-name="Layer 1"
                            ...
                            <path
                                ...
                                transform="translate(-168.64838 -127.60704)" fill="#e6e6e6" />
                            <path
                            ...
                            <path
                                ...
                                transform="translate(-168.64838 -127.60704)" fill="#ffb8b8" />
                            <path
                            ...
                            <path
                                ...
                                transform="translate(-168.64838 -127.60704)" fill="#ccc" />
                            <path
                            ...
                            <path
                                ...
                                transform="translate(-168.64838 -127.60704)" fill="#064e3b" />
                            <path
                            ...
                            <path
                                ...
                                transform="translate(-168.64838 -127.60704)" fill="#9e616a" />
                            <circle cx="584.91096" cy="94.03525" r="32.83012" fill="#9e616a" />
                            <path d="M599.36147,299.184" transform="translate(-168.64838 -127.60704)"
                                fill="#064e3b" />
                            <path
                     ...
                     <div class="text-center  mx-auto mt-5">
                            ...
                            <p class="mt-2">Silahkan klik Tombol <strong
                                    class="bg-emerald-800 p-1 rounded-lg text-white px-3">+ Donasi</strong><br/> Jika
                                Anda
                                ingin
                                melakukan donasi</p>
                        </div>
    ...
    <x-modal.card title="Tambah Donasi" blur wire:model.defer="modalDonationShow">
        ...
        <x-slot name="footer">
            <div class="flex justify-end gap-x-4">
                ...
                <div class="flex">
                    ...
                    <x-button primary label="Checkout Pembayaran" wire:click="checkout" />
                </div>
            </div>
        </x-slot>
    </x-modal.card>
    <x-modal.card title="Detail Donasi" blur wire:model.defer="modalDetailShow">
        @if ($detailData)
            <div class="grid md:grid-cols-2 gap-2 p-5">
                <div>
                    <small> Invoice</small>
                    <p>{{ $detailData['merchant_ref'] }}</p>
                </div>
                <div>
                    <small> Tipe Donasi</small>
                    <p class="uppercase">{{ $detailData['tipe_donasi'] }}</p>
                </div>
                <div>
                    <small> Jumlah Donasi</small>
                    <p>@currency($detailData['jumlah_donasi'] ?? 0)</p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <p>
                        {{ Carbon\Carbon::create($detailData['created_at'])->format('m-d-Y h:i') }}
